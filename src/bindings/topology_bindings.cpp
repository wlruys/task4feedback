...
void init_topology_ext(nb::module_ &m) {
  ...
  nb::class_<CommunicationStats>(m, "CommunicationStats")
      ...
      .def(nb::init<timecount_t, mem_t>(), "latency"_a,
           "bandwidth"_a) // Constructor with latency and bandwidth
  ...
  nb::class_<CommunicationRequest>(m, "CommunicationRequest")
      ...
      .def(nb::init<taskid_t, devid_t, devid_t, mem_t>(), "data_task_id"_a, "source"_a,
           "destination"_a, "size"_a)
  ...
  nb::class_<CommunicationNoise>(m, "CommunicationNoise")
      .def(nb::init<Topology &, unsigned int>(), "topology"_a, "seed"_a = 0,
           nb::keep_alive<1, 2>()) // Keep topology alive as long as noise object exists
  ...
  nb::class_<UniformCommunicationNoise, CommunicationNoise>(m, "UniformCommunicationNoise")
      .def(nb::init<Topology &, unsigned int>(), "topology"_a, "seed"_a = 0,
           nb::keep_alive<1, 2>()); // Keep topology alive as long as noise object exists
}