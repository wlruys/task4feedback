  ...
  size_t get_task_task_shared_read_edges(TorchInt64Arr1D &sources, TorchInt64Arr2D &output,
    ...
    if (global_output.shape(1) < output.shape(1)) {
      throw std::runtime_error(
          "Global edge output second dimension must be >= edge output second dimension");
    }
    ...
    std::vector<std::vector<dataid_t>> normalized_reads(sources_span.size());
    ...
    for (int64_t source_idx = 0; source_idx < static_cast<int64_t>(sources_span.size());
         source_idx++) {
