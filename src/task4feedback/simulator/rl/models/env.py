    ...
    *** RL state features ***
    ...
    def __init__(self, num_devices: int):
        ...
        self.device_utilization_state_offset = (
            self.task_property_offset + self.task_property_len
        )
        self.device_state_len = 3 * num_devices
        self.state_dim = self.device_utilization_state_offset + self.device_state_len
    ...
    def create_state(
        self, target_task: SimulatedTask, simulator, mapper: "RLTaskMapper"
    ):
    ...
    def create_global_info(self, current_state, simulator, mapper: "RLTaskMapper"):
        """
        ** Global information (2):
        ...
        # % of completed tasks
        current_state[0] = (
            sched_state.total_num_completed_tasks / mapper.total_num_tasks
            if mapper.total_num_tasks > 0
            else 0
        )
        ...
        current_state[1] = (
            convert_to_float(sched_state.time.scale_to("ms")) / (5 * target_exec_time)
        ...
        ** Task-specific information (5 + # devices):
        ...
        if mapper.max_indegree > 0:
            current_state[offset] = (
                mapper.get_indegree(target_task) / mapper.max_indegree
            )
        if logger.ENABLE_LOGGING:
            logger.runtime.debug(
                f"RL state [{offset}]: {current_state[offset].item()} "
        ...
        if mapper.max_outdegree > 0:
            current_state[offset] = (
                mapper.get_outdegree(target_task) / mapper.max_outdegree
            )
        if logger.ENABLE_LOGGING:
            logger.runtime.debug(
                f"RL state [{offset}]: {current_state[offset].item()} "
        ...
        if logger.ENABLE_LOGGING:
            ...
            logger.runtime.debug(
                f"RL state [{offset + target_task.info.func_id}]: "
        ...
        if mapper.max_duration > 0:
            ...
            task_duration = convert_to_float(
                sched_state.get_task_duration(
                    target_task, target_task.info.runtime.locations[0]
                ).scale_to("us")
            )
        ...
        for device in devicemap:
            ...
            if mapper.get_indegree(target_task) > 0:
                current_state[offset + dev_id] = current_state[
                    offset + dev_id
                ].item() / mapper.get_indegree(target_task)
            if logger.ENABLE_LOGGING:
                logger.runtime.debug(
                    f"RL state [{offset + dev_id}]: "
    ...
    def create_device_utilization_info(self, current_state, simulator):
        ...
        ** Device/network information (3 * # devices):
        ...
        if logger.ENABLE_LOGGING:
            logger.runtime.debug(
                f"RL state [{offset}]: {current_state[offset].item()} (Resource usage)."
            )
        ...
        for device in devicemap:
            ...
            if sched_state.total_active_workload > 0:
                current_state[dev_id + offset] = (
                    current_state[dev_id + offset].item()
                    / sched_state.total_active_workload
                )
            if logger.ENABLE_LOGGING:
                logger.runtime.debug(
                    f"RL state [{dev_id + offset}]: "
        ...
        for device, device_instance in devicemap.items():
            ...
            if is_nothing_launched:
                ...
                idle_time = convert_to_float(
                    device_instance.stats.idle_time.scale_to("ms")
                )
            norm_idle_time = (
                idle_time / (5 * target_exec_time) if target_exec_time else 0
            )
