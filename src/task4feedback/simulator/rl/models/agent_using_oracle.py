...
MappingLogs = namedtuple("MappingLogs", ("state", "p", "v", "pi"))
...
class SimpleAgent(RLModel):
    def __init__(
        ...
        self.optimizer = optim.RMSprop(
            self.network.parameters(), lr=0.00005
        )  # , weight_decay=0.5)
        # lr=0.0005)
    ...
    def select_device_using_p(self, f_action_probs, o_action_probs):
        ...
        self.num_consensus += (
            ...
            if o_action_probs[o_action] == o_action_probs[action]
            else 0
        )
        return action
    ...
    def select_device_using_pqr(self, state, v, f_action_probs, o_action_probs):
        ...
        if self.random_enabled:
            eps_threshold = self.eps_end + (self.eps_start - self.eps_end) * math.exp(
                -1.0 * self.action_steps / self.eps_decay
            )
            sample = random.random()
            self.action_steps += 1

        if not self.random_enabled or (self.random_enabled and sample > eps_threshold):
            action_probs = (1 - ld) * f_action_probs + ld * o_action_probs.to(
                self.device
            )
            o_action = max(enumerate(o_action_probs), key=lambda x: x[1])[0]
            ...
            self.num_consensus += (
                1
                if o_action == f_action
                else 1
                if o_action_probs[o_action] == o_action_probs[f_action]
                else 0
            )
            # print("o_action_probs action:", o_action, " f action:", f_action)
            action = torch.tensor(max(enumerate(action_probs), key=lambda x: x[1])[0])
            self.logs.append(
                MappingLogs(state, f_action_probs, v, o_action_probs.to(self.device))
            )
        else:
            # print("Random chosen")
            random.seed()
            action = torch.tensor(
                random.choice([d for d in range(self.num_actions)]), dtype=int
            )
        return action
    ...
    def select_device(
        ...
        if not self.is_training_mode():
            ...
            action = self.select_device_using_pqr(
                state, v, f_action_probs, o_action_probs
            )
        return action.item()
    ...
    def optimize_model(self, reward: float, sched_state: "SystemState"):
        ...
        concat_p = torch.cat([p.unsqueeze(0) for p in plist])
        concat_v = torch.cat(vlist).to(self.device)
        # Pi is not updated through this
        concat_pi = (
            torch.cat([pi.unsqueeze(0) for pi in pilist]).to(self.device).detach()
        )
