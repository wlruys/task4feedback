    ...
    def create_from_legacy_graph(graph, datamap):
        ...
        for i, task_id in enumerate(graph):
            ...
            if any_cpu_flag:
                ...
                vcu = int(placement_info.device_fraction * trip.MAX_VCUS)
...
class CompiledDefaultObserverFactory:
    def __init__(self, spec: trip.GraphSpec):
        self.spec = spec
        self.graph_extractor_t = trip.GraphExtractor
        self.task_feature_factory = trip.TaskFeatureExtractor
        self.data_feature_factory = trip.DataFeatureExtractor
        self.device_feature_factory = trip.DeviceFeatureExtractor
        self.task_task_feature_factory = trip.TaskTaskFeatureExtractor
        self.task_data_feature_factory = trip.TaskDataFeatureExtractor
    ...
    def create(self, simulator: Simulator):
        ...
        task_feature_extractor = self.task_feature_factory(
            trip.InDegreeTaskFeature,
            # trip.OutDegreeTaskFeature(state),
            # trip.OneHotMappedDeviceTaskFeature(state),
        )
        data_feature_extractor = self.data_feature_factory(
            trip.DataSizeFeature(state),
            # trip.DataMappedLocationsFeature(state),
        )

        device_feature_extractor = self.device_feature_factory(
            trip.DeviceArchitectureFeature(state),
            trip.DeviceIDFeature(state),
            # trip.DeviceMemoryFeature(state),
            # trip.DeviceTimeFeature(state),
        )

        task_task_feature_extractor = self.task_task_feature_factory(trip.TaskTaskSharedDataFeature(state))

        task_data_feature_extractor = self.task_data_feature_factory(
            # trip.TaskDataRelativeSizeFeature(state),
            trip.TaskDataUsageFeature(state),
        )
...
class DefaultObserverFactory(ExternalObserverFactory):
    def __init__(self, spec: trip.GraphSpec):
        ...
        # task_feature_factory.add(trip.TaskStateFeature)
        ...
        # device_feature_factory.add(trip.DeviceMemoryFeature)
        # device_feature_factory.add(trip.DeviceTimeFeature)
        ...
        # task_data_feature_factory.add(trip.TaskDataUsageFeature)
...
class SimulatorDriver:
    ...
    def __init__(
        ...
        Args:
            ...
            internal_mapper (trip.Mapper | Type[trip.Mapper], optional): The internal mapper instance or class. Defaults to trip.DequeueEFTMapper.
            ...
            simulator (Optional[trip.Simulator], optional): An optional simulator instance. If not provided, a new simulator will be created using the input and internal mapper.

        Attributes:
            input (SimulatorInput): The input data for the simulator.
            internal_mapper (trip.Mapper): The internal mapper instance.
            external_mapper (ExternalMapper): The external mapper instance.
            observer (ExternalObserver): The observer instance.
            simulator (trip.Simulator): The simulator instance.
        """
        self.input = input
