...
def _observation_to_heterodata_counted(
    ...
    hetero_data["vs_policy"].x = observation["aux", "vs_policy"].unsqueeze(0)
    ...
    for node_type, node_data in observation["nodes"].items():
        ...
        if store_node_counts:
            hetero_data[f"{node_type}_count"].x = torch.tensor(
...
class CandidateGNNObserver(CandidateObserver):
    ...
    def new_observation_buffer(self, spec=None):
        ...
        edge_tensor = TensorDict(
            {
                "tasks_tasks": _make_edge_tensor(
                    spec.max_candidates * spec.max_candidates,
                    self.task_task_features.feature_dim,
                )
            }
        )
        ...
        return TensorDict(
            {"nodes": node_tensor, "edges": edge_tensor, "aux": aux_tensor}
        )

    def shared_read_edges(self, output: TensorDict):
        ntasks = output["nodes", "tasks", "count"][0]
        count = self.graph_extractor.get_task_task_shared_read_edges(
            output["nodes", "tasks", "glb"][:ntasks],
            output["edges", "tasks_tasks", "idx"],
            output["edges", "tasks_tasks", "glb"],
        )
    ...
    @staticmethod
    def _edge_tensor_to_pairs(
        edge_tensor: torch.Tensor, count: int
    ) -> list[tuple[int, int]]:
        if count <= 0:
            return []
    ...
    def _candidate_read_sets(self, task_ids: list[int]) -> dict[int, set[int]]:
        ...
        if graph is None:
            raise RuntimeError(
                "Cannot debug shared-read edges: simulator.input.graph is unavailable."
            )
        ...
        raise RuntimeError(
            "Cannot debug shared-read edges: graph does not expose task read sets."
        )

    def _candidate_grid_coords(
        self, task_ids: list[int]
    ) -> tuple[dict[int, dict[str, int]], tuple[int, int] | None]:
        graph = getattr(getattr(self.simulator, "input", None), "graph", None)
        if graph is None:
            return {}, None
        ...
        for task_id in task_ids:
            ...
            if hasattr(graph, "xy_from_id"):
                ...
                and getattr(tasks[task_id], "tag", None) is not None
            ):
    ...
    def debug_shared_read_edges(
        self,
        output: TensorDict,
        dump_dir: str | None = None,
        max_shared_data_per_edge: int = 8,
    ) -> dict[str, Any]:
        n_candidates = int(output["nodes", "tasks", "count"][0].item())
        candidate_ids = [
            int(v) for v in output["nodes", "tasks", "glb"][:n_candidates].tolist()
        ]
        ...
        observed_global_pairs = set(
            self._edge_tensor_to_pairs(
                output["edges", "tasks_tasks", "glb"], observed_count
            )
        )
        observed_local_pairs = set(
            self._edge_tensor_to_pairs(
                output["edges", "tasks_tasks", "idx"], observed_count
            )
        )
        ...
        for src_idx in range(n_candidates):
            ...
            for dst_idx in range(src_idx + 1, n_candidates):
                ...
                shared_data_preview[f"{src_id}->{dst_id}"] = sorted(shared)[
                    : int(max_shared_data_per_edge)
                ]
        ...
        report: dict[str, Any] = {
            ...
            "read_counts": {
                str(task_id): len(read_sets.get(task_id, set()))
        ...
        if dump_dir is not None:
            ...
            sim_time = (
                int(output["aux", "time"][0].item()) if "time" in output["aux"] else -1
            )
            ...
            edge_lines = [
                "src_idx,dst_idx,src_task,dst_task,src_row,src_col,dst_row,dst_col,status"
            ]
            for src_idx, dst_idx in sorted(
                expected_local_pairs.intersection(observed_local_pairs)
            ):
                src_grid = grid_coords.get(
                    candidate_ids[src_idx], {"row": -1, "col": -1}
                )
                dst_grid = grid_coords.get(
                    candidate_ids[dst_idx], {"row": -1, "col": -1}
                )
                edge_lines.append(
            ...
            for idx, task_id in enumerate(candidate_ids):
                ...
                if grid["row"] >= 0 and grid["col"] >= 0:
                    dot_lines.append(
                        f'  n{idx} [label="{label}", pos="{grid["col"]},{-grid["row"]}!"];'
                    )
                else:
            ...
            for src_idx, dst_idx in missing_local:
                dot_lines.append(
                    f"  n{src_idx} -- n{dst_idx} [color=red, style=dashed];"
                )
            for src_idx, dst_idx in extra_local:
                dot_lines.append(
                    f"  n{src_idx} -- n{dst_idx} [color=blue, style=dotted];"
                )
            dot_lines.append("}")
            ...
            np.savetxt(
                out_dir / "observed_adj.csv",
                observed_adj_sym.numpy(),
                fmt="%d",
                delimiter=",",
            )
            np.savetxt(
                out_dir / "expected_adj.csv",
                expected_adj_sym.numpy(),
                fmt="%d",
                delimiter=",",
            )
