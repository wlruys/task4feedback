...
class RuntimeEnv(EnvBase):
    def __init__(
        ...
        self.animate_each_episode = animate_each_episode
        ...
        if self.permute_locations or permute_idx is not None:
            graph = simulator_factory.input.graph
            if (
                hasattr(graph, "get_cell_locations")
                and hasattr(graph, "set_cell_locations")
                and hasattr(graph, "permute_locations")
            ):
                self.legacy_graph = False

            if self.legacy_graph:
                raise RuntimeError("Legacy graphs do not support permuting locations.")
            ...
            self.location_map = {
                i: j for i, j in zip(self.location_list, self.location_list)
            }

            graph.permute_locations(self.location_map, self.permute_idx)
    ...
    def _reset(self, td: Optional[TensorDict] = None) -> TensorDict:
        ...
        if self.animate_each_episode:
            self.animate(
                frames=400,
                dpi=20,
                bitrate=50,
                title=f"{self.title}_{self.resets}",
            )
        ...
        if self.permute_locations:
            self.permute_idx = (self.permute_idx + 1) % self.max_permutes
            graph = self.simulator_factory.input.graph
            graph.permute_locations(self.location_map, self.permute_idx)
    ...
    def animate(self, frames=400, dpi=20, bitrate=50, title="env_animation"):
        sim_time = self.simulator.time
        time_interval = int(sim_time / frames)
        if sim_time > 0:
            animate_mesh_graph(
                self,
...
def wrap_runtime_env_for_external_mapper(
    env_type: Type[RuntimeEnv],
) -> Type[RuntimeEnv]:
    """
    Wraps the given environment type with an external mapper runtime environment.
    """

    class ExternalMapperRuntimeEnv(env_type):
    ...
    return ExternalMapperRuntimeEnv
