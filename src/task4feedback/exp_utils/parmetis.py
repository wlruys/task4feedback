...
def run_parmetis(
    ...
    def _log_layout(prefix: str, layout: list[int]) -> None:
        if not logger.isEnabledFor(logging.DEBUG):
            return

        row_buffer: list[str] = []
        for i, loc in enumerate(layout, start=1):
            row_buffer.append(str(loc))
            if i % width == 0:
                logger.debug("%s row %d: %s", prefix, i // width, " ".join(row_buffer))
                row_buffer = []
        if row_buffer:
            row_index = len(layout) // width + 1
            logger.debug("%s row %d: %s", prefix, row_index, " ".join(row_buffer))
    ...
    if rank == 0:
        ...
        if verbose:
            _log_layout("Initial partition", cell_to_device)
    while True:
        ...
        if rank == 0:
            ...
            if verbose:
                _log_layout("Post-ParMETIS partition", cell_to_device)
