...
def evaluate_and_save(policy: PolicyType, cfg: DictConfig, eval_state: EvalState, n_samples=20):
        ...
        for i in range(n_samples):
            ...
            print(eval_state.workloads, type(eval_state.workloads))
...
def check_eval(location: EvalLocation, cfg: DictConfig):
    ...
    print(f"Checking {n_samples} samples from {location.file_path}")

    print(f"Best policy: {eval_state.best_policy} with time {eval_state.best_time}")
    ...
    for i in range(n_samples):
        ...
        print(f" Sample {i}: Saved EFT time: {saved_time}, Simulated EFT time: {sim_time}")
        if not numpy.isclose(saved_time, sim_time, atol=1e-3):
            print(f"  Warning: EFT time changed! {saved_time} -> {sim_time}")
        else:
...
def configure_training(cfg: DictConfig):
    ...
    for policy in policies:
        ...
        evaluate_and_save(policy, cfg, eval_state, n_samples=1)
