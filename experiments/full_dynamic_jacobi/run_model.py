...
from experiment_helper.env import RuntimeEnv, make_env
from experiment_helper.graph import make_graph_builder
from experiment_helper.model import (
    create_td_actor_critic_models,
    load_policy_from_checkpoint,
)
from experiment_helper.run_name import make_folder_name
...
def find_model_dirs(
    cfg: DictConfig,
    search_start: int = int(1e9),
    search_stop: int = int(5e9),
    include_all_in_range: bool = False,
) -> list[Path]:
    """
    Find existing model directories around the current ``level_memory``.

    By default, this returns the nearest existing directory in the negative and
    positive directions. When ``include_all_in_range`` is ``True``, it returns
    every existing directory found within the search range instead of stopping
    at the first match in each direction.
    """
    default_mem = cfg.graph.config.level_memory
    folder_name, _, _, _ = make_folder_name(cfg, change_name=True)
    default_model_dir = Path(f"./models/{cfg.system.n_devices - 1}gpus") / folder_name

    found_dirs: list[Path] = []

    if default_model_dir.exists():
        found_dirs.append(default_model_dir)
        if not include_all_in_range:
            return found_dirs

    for direction in (-1, 1):
        matched_dir: Path | None = None
        for delta in range(search_start, search_stop + int(1e9), int(1e9)):
            candidate_mem = default_mem + (direction * delta)
            if candidate_mem <= 0:
                continue

            cfg.graph.config.level_memory = candidate_mem
            folder_name, _, _, _ = make_folder_name(cfg, change_name=True)
            model_dir = Path(f"./models/{cfg.system.n_devices - 1}gpus") / folder_name
            if model_dir.exists():
                if include_all_in_range:
                    found_dirs.append(model_dir)
                    continue

                matched_dir = model_dir
                break

        if matched_dir is not None:
            found_dirs.append(matched_dir)
...
def configure_training(cfg: DictConfig) -> None:
    default_mem = cfg.graph.config.level_memory
    default_folder_name, _, _, _ = make_folder_name(cfg, change_name=True)
    cfg.graph.config.steps = EVAL_GRAPH_STEPS
    print(f"Default model folder name: {default_folder_name}", flush=True)
    model_dirs = find_model_dirs(cfg)
    if not model_dirs:
        print(
            f"No model folders found within +/-[1e9, 100e9] of level_memory={default_mem}. Terminating.",
            flush=True,
        )
        return
    ...
    # with open(os.path.join("./norms", f"{default_folder_name}.pkl"), "wb") as f:
    #     pickle.dump(norm, f)
    norm = None
    for model_dir in model_dirs:
        for model_path in model_dir.glob("*.pt"):
            ...
            if csv_entry_exists(results_csv, key):
                print(
                    f"[SKIP] CSV entry already exists for {model_path.name}", flush=True
                )
            ...
            if norm is None:
                # Load normalization (or generate if it doesn't exist)
                norm_path = f"./norms/{default_folder_name}.pkl"
                if os.path.exists(norm_path):
                    with open(norm_path, "rb") as f:
                        norm = pickle.load(f)
                else:
                    graph_builder = make_graph_builder(cfg)
                    _, norm = make_env(graph_builder=graph_builder, cfg=cfg)
                    pickle.dump(norm, open(norm_path, "wb"))
