...
def configure_training(cfg: DictConfig):
    ...
    if OmegaConf.select(cfg, "graph.config.workload_args.traj_type") is not None:
        ...
        graph_name = "static"
    if "Dilation" in cfg.models.layers.state._target_:
        if "Uncond" in cfg.models.layers.state._target_:
            network = "UncondCNN"
        else:
            network = "CNN"
    elif "Vector" in cfg.models.layers.state._target_:
        network = "Vector"
    elif "GNN" in cfg.models.layers.state._target_:
        network = "GNN"
    else:
        print(cfg.models.layers.state._target_)
        raise ValueError("Unknown model type in cfg.models.layers.state._target_")
    ...
    for item in results:
        if rank == 0:
            ...
            if normalization is None:
                print("Normalization artifacts missing, initializing new stats")
                env, normalization = make_env(
                ...
                print("Loaded normalization stats from artifacts")
                env = make_env(
                    graph_builder=graph_builder,
                    cfg=cfg,
