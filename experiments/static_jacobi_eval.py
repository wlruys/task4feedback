...
def main(cfg: DictConfig):
    ...
    cfg.system.device_copyengines = 2
    # 2 (in + out) D2D
    cfg.system.d2d_links = 1
    # Every device pairs have 1 incoming + 1 outgoing link
    cfg.system.n_devices = 5
    # 1 CPU + 4 GPUs
    cfg.system.cpu_copyengines = 8
    ...
    for ir, br in [(0.1, 0.1), (1, 0.1), (1, 1), (10, 1)]:
        ...
        cfg.graph.config.boundary_size = 16 * 2**20  # 16MB
        cfg.graph.config.interior_size = cfg.graph.config.boundary_size * 4 * ir / br
        cfg.graph.config.compute_time = cfg.graph.config.interior_size / cfg.system.d2d_bw * 1e6 / ir
