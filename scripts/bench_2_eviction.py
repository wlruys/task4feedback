...
def build_graph(system):
    ...
    return DynamicJacobiGraph(geom, config, system=system)


def randomize_step0_locations(graph):
    # Reset step-0 ownership before applying this seed's random placement.
    n_cells = len(graph.data.geometry.cells)
    graph.set_cell_locations([0] * n_cells, step=0)
    # Device 0 is CPU; GPU ids are [1, N_DEVICES - 1].
...
def run_once(sim_input: SimulatorInput):
    ...
    task_runtime = fresh.state.get_task_runtime()
    ...
    return (
        ...
        evict_mv,
        n_data_events,
        n_data_movement_events,
        n_eviction_events,
        n_eviction_movement_events,
    )
...
def run_scenario(label, graph, system_mem, n_reps=N_REPS):
    ...
    last_evict = 0
    last_data_events = 0
    last_data_movement_events = 0
    last_eviction_events = 0
    last_eviction_movement_events = 0
    ...
    for rep in range(n_reps):
        ...
        last_evict = evict_mv
        last_data_events = n_data_events
        last_data_movement_events = n_data_movement_events
        last_eviction_events = n_eviction_events
        last_eviction_movement_events = n_eviction_movement_events
        print(
            f"    Rep {rep+1}/{n_reps}:  "
            ...
            f"data_events={n_data_events} (movement={n_data_movement_events})  "
            f"eviction_events={n_eviction_events} (movement={n_eviction_movement_events})"
        )
    ...
    print(
        ...
        f"data_events={last_data_events} (movement={last_data_movement_events})  "
        f"eviction_events={last_eviction_events} (movement={last_eviction_movement_events})"
    )
...
def main():
    ...
    parser.add_argument(
        "--rebuild-graph-per-seed",
        action="store_true",
        help="Rebuild the DynamicJacobiGraph for every seed (slow, previous behavior).",
    )
    ...
    print(f"Rebuild graph   : {args.rebuild_graph_per_seed}")
    print("=" * 60)

    gen_system = build_system()
    graph = None
    if not args.rebuild_graph_per_seed:
        t0 = time.perf_counter()
        graph = build_graph(gen_system)
        build_time = time.perf_counter() - t0
        print(f"Graph build (shared): {build_time:.1f}s   Tasks: {len(graph)}")
    ...
    for seed_offset in range(args.n_seeds):
        ...
        print(f"\nSeed {seed}")
        set_seed(seed)
        if args.rebuild_graph_per_seed:
            t0 = time.perf_counter()
            graph = build_graph(gen_system)
            build_time = time.perf_counter() - t0
            print(f"      Build time: {build_time:.1f}s   Tasks: {len(graph)}")

        randomize_step0_locations(graph)
