...
if __name__ == "__main__":
    ...
    def env_wrapper(config) -> RuntimeEnv:
        ...
        if rtc:
            ...
            input = SimulatorInput(
                m, d, s, transition_conditions=trip.DefaultTransitionConditions()
            )
        env = RuntimeEnv(
            SimulatorFactory(
                input,
    ...
    step = 4
    state_dict = torch.load(f"dqn_model_{step}.pth")
    model_explore.load_state_dict(state_dict)

    print(model_explore.modules)
    ...
    evaluate_loaded_model(
        ...
        rtc,
        animate=True,
        title=f"dqn_random_{step}_{rtc}",
