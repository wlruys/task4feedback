...
from task4feedback.trip.interface import (
    SimulatorHandler,
...
from task4feedback.trip.models import (
...
class Args:
    ...
    wandb_run_name = f"a2c_{env_id}_{steps}steps_{width}width_{dimensions}d_RandALL"
...
def initialize_simulator(seed=0):
    ...
    bandwidth = 10 * 1024 * 1024 * 1024 / (10**4)
    latency = 1
...
def replay(batch_info, h, optimizer, global_step):
    ...
    for k in range(args.replay_batches):
        ...
        for i, batchelem in enumerate(loader):
            ...
            entropy_loss = (dentropy * mask).sum()
            entropy_loss = entropy_loss / max(num_valid, 1)

            loss = (dpg_loss) - args.ent_coef * entropy_loss + v_loss * args.vf_coef
