...
def build_mem_range(start_mem, end_mem, step_mem):
    mem_range = []
    if start_mem > 0 and end_mem > 0:
        curr = start_mem
        while curr <= end_mem:
            mem_range.append(curr)
            curr += step_mem
    return mem_range


def resolve_mem_points(params, default_start_mem, default_end_mem, default_step_mem):
    has_explicit_mem = "mem" in params
    has_local_start = "start_mem" in params
    has_local_end = "end_mem" in params
    has_local_step = "step_mem" in params

    if has_explicit_mem and (has_local_start or has_local_end):
        raise ValueError(
            "Experiment config cannot define both 'mem' and 'start_mem'/'end_mem'."
        )

    if has_local_start != has_local_end:
        raise ValueError(
            "Experiment config must define both 'start_mem' and 'end_mem' "
            "when overriding the default memory range."
        )

    if has_explicit_mem:
        mem_value = params["mem"]
        if isinstance(mem_value, list):
            return [parse_mem(m) for m in mem_value]
        return [parse_mem(mem_value)]

    start_mem = default_start_mem
    end_mem = default_end_mem
    step_mem = default_step_mem

    if has_local_start and has_local_end:
        start_mem = parse_mem(params["start_mem"])
        end_mem = parse_mem(params["end_mem"])
        if has_local_step:
            step_mem = parse_mem(params["step_mem"])
    elif has_local_step:
        step_mem = parse_mem(params["step_mem"])

    return build_mem_range(start_mem, end_mem, step_mem)
    ...
    for seed_offset in range(num_seeds):
        ...
        for exp_name, run_list in config["experiments"].items():
            for params in run_list:
                ...
                final_mem_points = resolve_mem_points(
                    params,
                    start_mem,
                    end_mem,
                    step_mem,
                ...
                param_context, param_sweeps = split_sweep_fields(
                    params,
                    skip_keys={"mem", "start_mem", "end_mem", "step_mem", "sweeps"},
                )
