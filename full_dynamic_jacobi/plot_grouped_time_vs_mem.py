...
def build_mapper_summary_csv(
    ...
    def compute_percent_increase(value: float | None, baseline: float | None) -> str:
        if value is None or baseline is None or baseline <= 0:
            return "n/a"
        return f"{((value / baseline) - 1.0) * 100.0:.0f}"
...
def read_rows(
    ...
    return series_by_group, zero_eviction_points, base_lookup


def resolve_results_paths(
    n_gpus: int, include_rl: bool, noise: bool = False, extended: bool = False
) -> tuple[Path, Path | None]:
    results_dir = RESULTS_ROOT / f"{n_gpus}gpus"
    ...
    rl_csv_name = (
        f"noise_results_rl_{steps}.csv" if noise else f"results_rl_{steps}.csv"
    )
...
def resolve_normalization_path(n_gpus: int) -> Path:
    return RESULTS_ROOT / f"{n_gpus}gpus" / "normalization.csv"
...
def prepare_plot_data(
    ...
    grouped_rows, zero_eviction_points, base_lookup = read_rows(
    ...
    if args.extend and baseline_mems is None:
        ...
        baseline_mems = load_normalization_baselines(
            normalization_csv_path, base_lookup
        )
    ...
    return grouped_rows, zero_eviction_points, baseline_mems, y_axis_tops
...
def run_for_ngpus(args: argparse.Namespace, n_gpus: int) -> None:
    ...
    if args.extend:
        normalization_csv_path = (
            resolve_normalization_path(n_gpus).expanduser().resolve()
        )
    ...
    output_dir = PLOTS_ROOT / f"{n_gpus}gpus"
    ...
    grouped_rows, zero_eviction_points, baseline_mems, y_axis_tops = prepare_plot_data(
    ...
    if args.noise:
        ...
        noise_grouped_rows, noise_zero_eviction_points, _, noise_y_axis_tops = (
            prepare_plot_data(
                noise_csv_path,
                noise_rl_csv_path,
                normalization_csv_path,
